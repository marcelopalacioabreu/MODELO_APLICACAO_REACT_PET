...
echo %INFO% Instalando dependências do frontend (se necessário)...
call npm --prefix frontend install
echo %INFO% Executando build do frontend (pré-compilação)...
call npm --prefix frontend run build
if errorlevel 1 (
	echo %ERR% Build do frontend falhou. Verifique logs em frontend\. Saindo.
	exit /b 1
)
echo %OK% Build do frontend concluído.
echo %INFO% Iniciando frontend (next start - modo produção)...
start "frontend" cmd /k "cd /d frontend && set NODE_ENV=production && call npm run start"
